...
def root():
    return {
        "status": "online",
        "service": "ByteForge Landslide Risk Early Warning Prototype",
        "version": "1.0.0",
        "endpoints": [
...
def simulate_event(event_id: str, token: DeviceTokenIn):
    ...
    csv_candidates = [
        os.path.join(os.path.dirname(__file__), "..", "uttarakhand_landslide_events.csv"),
        os.path.join(os.path.dirname(__file__), "uttarakhand_landslide_events.csv"),
        os.path.join(os.getcwd(), "uttarakhand_landslide_events.csv"),
    ]
    csv_path = None
